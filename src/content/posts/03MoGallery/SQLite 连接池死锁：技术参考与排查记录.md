...
1. `listAssets`：在调用 `loadAssetOrganization` 前显式 `rows.Close()`，先释放连接再发起嵌套查询（保留 `defer` 覆盖提前返回路径）。`rows.Err()` 在 Close 后仍可读取，不改变语义。
2. 连接池放宽：`SetMaxOpenConns(4 → 8)`、`SetMaxIdleConns(2 → 4)`，给多并发读留余量（WAL 下多读并发无写入冲突）。
